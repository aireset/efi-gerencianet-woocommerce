...
function init_gerencianet_pix() {
	...
	class WC_Gerencianet_Pix extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->method_title       = __( 'Efi - Pix', 'efigerencianet-por-aireset' );
			$this->method_description = __( 'Venda com pix através da Efi.', 'efigerencianet-por-aireset' );
			...
			$discountText = '';
			if ( $this->get_option( 'pix_discount' ) != '' && $this->get_option( 'pix_discount' ) != '0' ) {
				$discountText = ' - ' . $this->get_option( 'pix_discount' ) . '% de Desconto';
			}

			// Load the settings.
			$this->init_settings();
			$this->title       = __( 'Efi - Pix', 'efigerencianet-por-aireset' ) . $discountText;
			$this->description = __( 'Aceite pix através da Efi.', 'efigerencianet-por-aireset' );
			$this->enabled     = $this->get_option( 'pix' );

			$this->pix_key               = sanitize_text_field( $this->get_option( 'pix_key' ) );
			$this->pix_file              = sanitize_text_field( $this->get_option( 'pix_file' ) );
			$this->pix_discount          = sanitize_text_field( $this->get_option( 'pix_discount' ) );
			$this->pix_discount_shipping = sanitize_text_field( $this->get_option( 'pix_discount_shipping' ) );
			$this->pix_number_hours      = sanitize_text_field( $this->get_option( 'pix_number_hours' ) );
			$this->pix_mtls              = sanitize_text_field( $this->get_option( 'pix_mtls' ) );
		...
		public function init_form_fields() {
			$certificateLabel = $this->get_option( 'pix_file' ) != '' ? 'Certificado Pix salvo:<code>'.$this->get_option( 'pix_file_name' ).'</code>' : 'Nenhum certificado salvo';
			...
			$this->form_fields = array(
				...
				'one_payment_section'        => array(
					'title' => __( 'Configurações de recebimento', 'efigerencianet-por-aireset' ),
					'type'  => 'title',
				),
				'pix'                        => array(
					'title'   => __( 'Pix', 'efigerencianet-por-aireset' ),
					'type'    => 'checkbox',
					'label'   => __( 'Habilitar Pix', 'efigerencianet-por-aireset' ),
					'default' => 'no',
				),
				'pix_key'                    => array(
					'title'       => __( 'Chave Pix', 'efigerencianet-por-aireset' ),
					'type'        => 'text',
					'description' => __( 'Insira sua chave Pix da Gerencianet', 'efigerencianet-por-aireset' ),
					'desc_tip'    => false,
					'placeholder' => '',
					'default'     => '',
				),
				'pix_file'                   => array(
					'title'       => __( 'Certificado Pix', 'efigerencianet-por-aireset' ),
					'type'        => 'file',
					'description' => $certificateLabel,
					'desc_tip'    => false,
				),
				'pix_discount'               => array(
					'title'       => __( 'Desconto no Pix', 'efigerencianet-por-aireset' ),
					'type'        => 'number',
					'description' => __( 'Desconto a ser aplicado para pagamentos com Pix. (Deixe em branco ou como 0 para não aplicar desconto)', 'efigerencianet-por-aireset' ),
					'placeholder' => '0',
					'default'     => '0',
				),
				'pix_discount_shipping'      => array(
					'title'       => __( 'Modo de desconto', 'efigerencianet-por-aireset' ),
					'type'        => 'select',
					'description' => __( 'Escolha como aplicar o desconto definido na opção anterior.', 'efigerencianet-por-aireset' ),
					'default'     => 'total',
					'options'     => array(
				...
				'pix_number_hours'           => array(
					'title'       => __( 'Expiração do pix', 'efigerencianet-por-aireset' ),
					'type'        => 'number',
					'description' => __( 'Em quantas horas o Pix expira depois de emitido', 'efigerencianet-por-aireset' ),
					'placeholder' => '0',
					'default'     => '24',
				),
				'pix_mtls'                   => array(
					'title'       => __( 'Validar mTLS', 'efigerencianet-por-aireset' ),
					'type'        => 'checkbox',
					'description' => __( 'Entenda os riscos de não configurar o mTLS <a href="https://dev.gerencianet.com.br/docs/api-pix-endpoints#webhooks" target="_blank">clicando aqui.</a>', 'efigerencianet-por-aireset' ),
					'default'     => 'no',
				),
			);
		}
		...
		public function savePixCertificate() {
			$file_name = $_FILES['woocommerce_WC_Gerencianet_Pix_pix_file']['name'];
			
			if ( $_FILES['woocommerce_WC_Gerencianet_Pix_pix_file']['error'] != 0 ) {
				return;
			}
			...
			switch ($fileActualExt) {
				case 'pem':
					if ( ! $file_read = file_get_contents( $_FILES['woocommerce_WC_Gerencianet_Pix_pix_file']['tmp_name'] ) ) { // Pega o conteúdo do arquivo
						echo '<div class="error"><p><strong> Falha ao ler arquivo o Certificado Pix! </strong></div>';
						return;
					}
					break;
				case 'p12':
					if ( ! $cert_file_p12 = file_get_contents( $_FILES['woocommerce_WC_Gerencianet_Pix_pix_file']['tmp_name'] ) ) { // Pega o conteúdo do arquivo
						echo '<div class="error"><p><strong> Falha ao ler arquivo o Certificado Pix! </strong></div>';
						return;
					}
			...
			if ( isset( $file_read ) ) {
				$this->update_option( 'pix_file_name', $file_name );
				$this->update_option( 'pix_file', $file_read );
			}
		...
		public function payment_fields() {
			...
			if(intval( $this->get_option( 'pix_discount' ) ) > 0){
				$discountMessage = '';
				if ( $this->get_option( 'pix_discount_shipping' ) == 'total' ) {
				...
				$discountWarn = '<div class="warning-payment" id="wc-gerencianet-messages-sandbox">
										<div class="woocommerce-info">' .esc_html( $this->get_option( 'pix_discount' ) ) . '% de Desconto'.$discountMessage. '</div>
									</div>';
			...
			<div class="form-row form-row-wide" id="gn_field_pix">
				...
				<input id="pix_cpf_cnpj" name="pix_cpf_cnpj" type="text" placeholder="___.___.___-__" autocomplete="off" onkeypress="return event.charCode >= 48 && event.charCode <= 57">
			</div>
			<div class="clear"></div>
			</fieldset>
			<?php
		...
		public function validate_fields() {
			if ( empty( sanitize_text_field( $_POST['pix_cpf_cnpj'] ) ) ) {
				wc_add_notice( __( 'O CPF é obrigatório', 'efigerencianet-por-aireset' ), 'error' );
				return false;
			} else {
				$cpf_cnpj = sanitize_text_field( $_POST['pix_cpf_cnpj'] );
		...
		public function process_payment( $order_id ) {
			...
			if ( $this->get_option( 'pix_discount' ) != '' && $this->get_option( 'pix_discount' ) != '0' ) {
				...
				if ( $this->get_option( 'pix_discount_shipping' ) == 'total' ) {
					$discount = ( ( $value ) * ( intval( $this->get_option( 'pix_discount' ) ) / 100 ) );
					$discountMessage = ' no valor total da compra';
				} else {
					$discount = ( ( $value - $shippingTotal ) * ( intval( $this->get_option( 'pix_discount' ) ) / 100 ) );
					$discountMessage = ' no valor total dos produtos (frete não incluso)';
				}
				...
				$order_item_id = wc_add_order_item(
					$order_id,
					array(
						'order_item_name' => $this->get_option( 'pix_discount' ) . __( '% de desconto no Pix' ).$discountMessage,
						'order_item_type' => 'fee',
					)
				);
				if ( $order_item_id ) {
			...
			$cpf_cnpj = str_replace( '.', '', sanitize_text_field( $_POST['pix_cpf_cnpj'] ) );
			...
			if ( Gerencianet_Validate::cpf( $cpf_cnpj ) ) {
				...
				wc_add_notice( __( 'CPF/CNPJ inválido.', 'efigerencianet-por-aireset' ), 'error' );
				return false;
			}

			$body = array(
				'calendario'     => array( 'expiracao' => intval( $this->get_option( 'pix_number_hours' ) ) * 3600 ),
				'devedor'        => $customer,
				'valor'          => array( 'original' => sprintf( '%0.2f', $value ) ),
				'chave'          => $this->get_option( 'pix_key' ),
			...
			try {
				...
				update_post_meta( $order_id, '_pix_link', $linkPix );
				update_post_meta( $order_id, '_pix_qrcode', $qrCode['imagemQrcode'] );
				update_post_meta( $order_id, '_pix_copy', $qrCode['qrcode'] );
				update_post_meta( $order_id, '_pix_txid', $charge['txid'] );
		...
		public function successful_webhook( $posted ) {
			...
			foreach ( $orders as $order ) {
				...
				if ( isset( $pix[0]['txid'] ) && $pix[0]['txid'] != '' && ( get_post_meta( $order->get_id(), '_pix_txid', true ) == $pix[0]['txid'] ) ) {
					add_post_meta( intval( $order->get_id() ), '_pix_E2EID', $pix[0]['endToEndId'], true );
		...
		public function add_view_payment_methods( $order ) {
			...
			if ( get_post_meta( $order->get_id(), '_pix_copy', true ) || get_post_meta( $order->get_id(), '_pix_link', true ) ) {
				?>
					<style>
			...
			<script>
				...
				function gncopy($field) {
					...
					navigator.clipboard.writeText('<?php echo esc_html( get_post_meta( $order->get_id(), '_pix_copy', true ) ); ?>');
					setTimeout(()=> {
				...
				function viewGnInfos(params) {
					Swal.fire({
						...
						icon: 'info',
						html: '<div class="gngrid-container"><div class="gngrid-item"><?php if ( get_post_meta( $order->get_id(), '_pix_copy', true ) !== NULL ) { ?><div class="gn-item-area"><img style="width:150px;" src=" <?php echo esc_url(plugins_url( 'woo-gerencianet-official/assets/img/pix-copia.png' )); ?> " /><br> <a onclick="gncopy(2)" id="gnpix" class="button gn-btn">Copiar Pix Copia e Cola</a> </div><?php }?></div><div class="gn-item-area"><?php if ( get_post_meta( $order->get_id(), '_pix_link', true ) !== NULL ) { ?> <div class="gngrid-item"><img style="width:150px;" src=" <?php echo esc_url(plugins_url( 'woo-gerencianet-official/assets/img/boleto-online.png' )); ?> " /><br><a href=" <?php echo esc_url(get_post_meta( $order->get_id(), '_pix_link', true )); ?>  " target="_blank" class="button gn-btn">Acessar Pix Online</a></div> <?php }?> </div></div>',
			...
			<div class="gn_payment_methods">
				<a onclick="viewGnInfos()" style="background: #EB6608; color:#ffff; border:none;" class="button">
					...
					<?php echo __( 'Visualizar Métodos de Pagamento', 'efigerencianet-por-aireset' ); ?>
				</a>
			</div>
